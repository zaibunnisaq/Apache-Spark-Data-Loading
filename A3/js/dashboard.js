...
// Process data for force-directed graph
function processForceDirectedData(dims) {
    const { nodes: nodeDim, links: linksDim, strength: strengthDim } = dims;
    
    // Get unique nodes
    const uniqueNodes = [...new Set(rawData.map(item => item[nodeDim]))];
    const nodes = uniqueNodes.map(name => ({
        id: name,
        group: 1,
        value: d3.mean(rawData.filter(d => d[nodeDim] === name).map(d => d[strengthDim])) || 1
    }));
